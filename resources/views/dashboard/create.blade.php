...
@section('content')
    <p class="h1">Create new Books</p>
    <form action="/dashboardsBooks" class="mt-4" method="POST">
        @csrf
        ...
        <div class="input-group flex-nowrap my-3" style="max-width: 100vh;">
            <input type="text" class="form-control border @error('title') is-invalid @enderror" placeholder="Title"
                name="title" aria-describedby="addon-wrapping" value="{{ @old('title') }}" required autofocus>
        </div>
        @error('title')
        ...
        <div class="input-group flex-nowrap my-3" style="max-width: 100vh;">
            <input type="text" class="form-control border @error('slug') is-invalid @enderror" placeholder="Slug"
                name="slug" aria-describedby="addon-wrapping" value="{{ @old('slug') }}"" required>
        </div>
        @error('slug')
            <div class="invalid-feedback">
                {{ $message }}
        ...
        <div class="input-group flex-nowrap my-3" style="max-width: 100vh;">
            <input type="number" class="form-control border @error('amount') is-invalid @enderror" placeholder="Amount"
                name="amount" aria-describedby="addon-wrapping" value="{{ @old('amount') }}" required>
        </div>
        @error('amount')
            <div class="invalid-feedback">
                {{ $message }}
